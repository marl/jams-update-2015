...
In JAMS1, there are four basic data types: \emph{observation}, \emph{event}, \emph{range}, and \emph{time series}.
...
tasks (see \cref{sec:schema:task}) generally use different time indexes, so the practitioner must both be aware of which index is 
used for any given task, and write code to handle it accordingly.  Moreover, it becomes non-trivial to temporally align annotations across
different tasks, since they must be mapped into a common representation.

JAMS2 simplifies this by reducing all observation types to a single format: regardless of task, each \emph{observation} consists of a 
...
By default, this simulates the \emph{range} type of JAMS1, but taking \emph{duration}$=0$ recovers the \emph{event}
type as well, with a small amount of redundancy.

The \emph{time series} type of JAMS1 can be viewed as an efficiently coded, dense sequence of \emph{range} observations with implicit
durations.
Recall that in JAMS1, each \emph{Annotation} object contains a list of observations in its \emph{data} field.
...
JAMS2 circumvents this by allowing a distinction between \emph{sparse} and \emph{dense} observation lists.
...
While the column-major format is generally more spatially efficient, the row-major format is more legible, 
and generally preferable for most tasks.
...
definitions. (For instance, time series now have explicit durations/sampling rates, and gaps are permitted.)
...
in JAMS1.
This is because all observations in JAMS2 are required to have a time and (potentially 0) duration.
However, we argue that this is an advantage for three reasons.  First, it is possible to have full-track observations by setting
\emph{time}$=0$ and $\emph{duration}$ to the full track duration.  Second, in reality, every observation type may vary over time, so the
schema should support this.  Finally, it forces the annotator to be explicit about the valid timing of an observation, and facilitates
partial annotation (see \cref{sec:future}).

\subsection{Tasks and namespaces}\label{sec:schema:task}
...
JAMS2 resolves these issues by adopting an an\-notation-major (rather than task-major) structure, and introducing the \emph{namespace}
abstraction.

Instead of grouping annotations by task at the top-level, a JAMS2 object contains a single list of \emph{Annotations}.
...
Each annotation object declares its task through the \emph{namespace} field.
A \emph{namespace} in JAMS2 is simply a partial schema declaration which defines the following properties:
\begin{itemize}
    \item an identifier, \emph{e.g.}, ``beat'' or ``tag\_cal500'';
    \item schema declarations for the \emph{value} and \emph{confidence} fields;
    \item whether data should be encoded in \emph{dense} or \emph{sparse} form.
\end{itemize}
...
not possible in the JAMS1 schema.
